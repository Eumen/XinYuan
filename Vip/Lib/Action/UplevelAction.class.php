...
// 			$s9=array('普通会员','铜级','银级','金级','蓝宝石','绿宝石','红宝石','珍珠','钻石','双钻石','黑钻石','皇冠');
	...
	public function MenberJinjiConfirm(){
		if ($_SESSION['Urlszpass'] == 'Myssjinji'){
			...
			// 升级差额
			$upCount = $newlevel - $fck_rs['cpzj'];
			if($us_money < $upCount){
			    $this->error('电子币不足，请先充值！');
			    exit;
			}
			// 查询参数设置数据
			$fee = M ('fee');
			...
			$s9 =explode('|',$fee_rs['s9']);
			//单量
			$danshu = bcdiv($upCount ,$s9[0]);
			if($fck_rs['cpzj'] >= $newlevel){
				$this->error('升级要高于原来等级！');
			}
		   ...
		   $time=time();
	        ...
	        $data['money']				= $upCount;//补差额
	        $data['u_level']			= $fck_rs['cpzj'];//旧的
	        $data['up_level']			= $newlevel;//新的
